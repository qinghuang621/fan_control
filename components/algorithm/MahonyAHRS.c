...
/* 积分项幅度上限：单位 rad/s 量级的等效角速度偏置。
 * 取 1.0f 表示最多允许 1 rad/s(≈57°/s) 的积分补偿量 —— 足以抵消陀螺零偏，
 * 又不至于在剧烈机动时把姿态拉飞。 */
...
/* 采样频率，默认 1000Hz；由 INS 任务在启动时按真实周期设置 */
static volatile float s_sampleFreq = 1000.0f;
...
void MahonyAHRSupdate(float q[4], float gx, float gy, float gz, float ax, float ay, float az, float mx, float my, float mz) {
	...
	if(!((ax == 0.0f) && (ay == 0.0f) && (az == 0.0f))) {
		...
		if(twoKi > 0.0f) {
			...
			integralFBz += twoKi * halfez * (1.0f / s_sampleFreq);
			/* ---- 积分限幅（本项目改动）---- */
			integralFBx = clampf(integralFBx, -INTEGRAL_LIMIT, INTEGRAL_LIMIT);
			integralFBy = clampf(integralFBy, -INTEGRAL_LIMIT, INTEGRAL_LIMIT);
			integralFBz = clampf(integralFBz, -INTEGRAL_LIMIT, INTEGRAL_LIMIT);
			gx += integralFBx;	// apply integral feedback
			gy += integralFBy;
			gz += integralFBz;
		}
		else {
			integralFBx = 0.0f;	// 积分未启用时保持零，避免残留
			integralFBy = 0.0f;
			integralFBz = 0.0f;
		}
	...
	gx *= (0.5f * (1.0f / s_sampleFreq));		// pre-multiply common factors
