...
| 《DM-S2325-1EC 减速电机（含 DM3520-1EC 驱动器）使用说明书 V1.0》 | DAMIAO 官方文档站：<https://wiki.aifitlab.com/damiao-docs/dm-s2325-1ec-v10-motor-instruction-manual> |
| DAMIAO 文档总入口（各机型说明书 / 上位机手册） | <https://wiki.aifitlab.com/damiao-docs> |

> 上述 PDF 均**不入库**（`.gitignore` 已排除 `RoboMaster*.pdf`、`DM-S2325*.pdf`），
> 需要时按上面的链接自行下载。本仓库工作区里保留的 `DM-S2325-1EC…V1.0 9.16.pdf` 仅供本地查阅。
> 引脚定义、CAN 口线序、加热电路规格等均以该手册与代码为准。
>
> **DM-S2325-1EC 关键结论**（详见 `计划2.md` §1.1.1，免得每次翻 PDF）：
> POS/VEL/T 均为**输出轴（减速后）**量；**上电 POS 固定 0.0 rad**；
> **量程硬编码 `12.5 / 200 / 10`**（`bsp_modbus.c`，抄自 running）；
> ⚠️ **POS 量程只有 ±12.5 rad → 必然回绕**，做里程必须做差分补偿。
...
| RS485_RE/DE | **PG8（未引出）** | 高=发送，低=接收。⚠️ **C 板并未把 PG8 引到任何连接器** —— 官方手册的 UART 口只有 `PG14`/`PG9` 两脚，查不到 PG8（它只是 MCU 的 BGA 焊球）。当前**方向由外部 TTL-RS485 模块的自动收发电路承担**；日后换用带 `DE` 脚的模块，可从板内 PG8 焊盘飞线启用 |
...
>
> ⚠️ **但 PG8 在 C 板上未引出**（见 §2.2 接线表）：这段方向控制逻辑**当前在硬件上不起作用**，
> 方向完全由外部模块的自动收发电路决定。代码**保留** —— 换用带 `DE` 脚的模块后飞线即可生效，无需改代码。
...
底盘为**正交全向轮**：LF/RR 是 X 轮（只吃 vx），RF/LR 是 Y 轮（只吃 vy），中心布局 **0.36 m × 0.36 m**，轮半径 **0.0375 m**（2026-09-23 实车实测；⚠️ 若"轮径 37.5 mm"实际指**直径**，则半径应为 0.01875，全部轮速翻倍，待确认）。**最大轮速 ±50 rad/s，等比例限幅**（纯安全钳位）。⚠️ 新几何下上位机 Fast 档最坏组合 vx=2.0 / vy=1.2 / wz=4.0 会到 **72.5 rad/s**、**触发限幅**（缩到约 0.69 倍）；旧几何（0.15 / 0.15 / 0.05）时该组合只到 46.0 rad/s、全程不触发。50 这个上限由电机额定 600 rpm ≈ 62.8 rad/s 反推得出，**不要为提高 Fast 档速度而突破它**。
...
- FreeRTOS 5 任务（ModbusTask / InsTask / FanTask / PulseTask / LedTask），1 ms SysTick + 1 ms TIM6 HAL 时基
...
- **恒温 PID 提前切闭环（2026-09-16 实现，2026-09-17 实测验证）**：温度一到 `目标 − 2℃` 就交 PID，
  不再等"连续确认达标"（原实现那段确认窗口里仍在满功率加热）。
  **改前过冲 49℃ → 改后峰值 46.9℃ 即回落**，已接近物理下限：完全停加热的阈值是
  `目标 + Iout/Kp = 45 + 2250/1600 = 46.4℃`，剩下那 0.5℃ 是"无制冷手段"下的热惯性，
  **除非降低达标前的升温速率，否则压不下去**。
...
- ✅ **恒温加热（2026-09-16 已定位并解决）**：**24V 正常供电下约 2 s 升到 45℃ 目标**（实测）。
  ...
  **不是加热电路的问题**。
  ...
  （另：`INS_TEMP_CONFIRM_CNT`=30 次（30×50ms=1.5s）只用于判定「**何时可以对外发姿态快照**」，
  2026-09-16 由 200 次下调。**它不决定加热方式** —— 加热早在 `目标 − 2℃` 时就交给 PID 了。）
